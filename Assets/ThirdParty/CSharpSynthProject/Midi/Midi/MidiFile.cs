    ...
    public class MidiFile
    {
        ...
        private static MidiTrack ReadTrack(BinaryReader reader)
        {
            ...
            while (reader.BaseStream.Position < endPosition)
            {
                ...
                {//voice message
                    prevStatus = status;
                    eventList.Add(ReadVoiceMessage(reader, delta, totalTime, status, reader.ReadByte()));
                ...
                {//data bytes
                    ...
                    {//otherwise apply running status
                        eventList.Add(ReadVoiceMessage(reader, delta, totalTime, prevStatus, status));
                        TrackVoiceStats(eventList[eventList.Count - 1], instList, drumList, ref channelList, ref noteOnCount);
                    }
                }
            }
        ...
        private static MidiEvent ReadVoiceMessage(BinaryReader reader, int delta, int totalTime, byte status, byte data1)
        {
            switch (status & 0xF0)
            {
                case 0x80: //NoteOff
                    return new MidiEvent(delta, status, data1, reader.ReadByte(), totalTime);
                case 0x90: //NoteOn
                    byte velocity = reader.ReadByte();
                    if (velocity == 0) //actually a note off event
                        status = (byte)((status & 0x0F) | 0x80);
                    return new MidiEvent(delta, status, data1, velocity, totalTime);
                case 0xA0: //AfterTouch
                    return new MidiEvent(delta, status, data1, reader.ReadByte(), totalTime);
                case 0xB0: //ControlChange
                    return new MidiEvent(delta, status, data1, reader.ReadByte(), totalTime);
                case 0xC0: //ProgramChange
                    return new MidiEvent(delta, status, data1, 0, totalTime);
